...
# Fix Pipeline Failures Script
# This script addresses all the issues causing pipeline failures

set -e

echo "🔧 Fixing Terraform Pipeline Failures..."
...
if [ "$VPC_COUNT" -ge 5 ]; then
    echo "⚠️  VPC limit reached! Using existing VPCs instead of creating new ones."
    echo "✅ VPC configuration updated to use existing resources"
else
...
echo ""
echo "💡 If you still encounter VPC limit issues:"
echo "1. Delete unused VPCs: aws ec2 delete-vpc --vpc-id <vpc-id>"
echo "2. Request VPC limit increase from AWS Support"
echo "3. Use existing VPCs with data sources"
